import { buildCookiePreviewScan, buildCookieExportFromScan, previewCookieSources, } from './cookie-export.js';
import { getCommonServiceSlugs } from './service-rules.js';
...
const importCommonFileInput = getElement('import-common-file');
const commonProfileStatus = getElement('common-profile-status');
const scanModeInputs = Array.from(document.querySelectorAll('input[name="scan-mode"]'));
...
async function loadSelectedServices() {
    return await new Promise((resolve) => {
        chrome.storage.local.get([SELECTION_KEY], (items) => {
            ...
            resolve(Array.isArray(value) ? value.filter((item) => typeof item === 'string') : []);
        });
    });
}
...
function buildPreviewDiagnosticLog(bundle) {
    const lines = [
...
function clearPreviewAfterExport(message) {
    currentScan = null;
    currentBundle = null;
    selectedServices = new Set();
    renderServices();
...
async function exportCommonProfile() {
    await runWithBusyState('正在导出常用配置...', async () => {
        ...
        await downloadBlob(new Blob([stringifyCommonProfile(profile)], { type: 'application/json;charset=utf-8' }), filename);
...
function buildCommonProfile(serviceSlugs, savedAt = new Date().toISOString()) {
    return {
        version: 1,
        savedAt,
...
function parseCommonProfile(value) {
...
function normalizeServiceSlugs(value) {
    if (!Array.isArray(value))
        return [];
    return [...new Set(value.filter((item) => typeof item === 'string').map((item) => item.trim()).filter(Boolean))];
}
function stringifyCommonProfile(profile) {
...
async function runWithBusyState(message, action) {
    setBusy(true);
...
function getLoginStatusText(summary) {
    ...
    if (summary.loginStatus === 'strong')
        return '登录态较完整';
    if (summary.loginStatus === 'partial' && summary.missingStrongMarkers.length === 0)
        return '已发现 Cookie';
    if (summary.loginStatus === 'partial')
        return '可能不完整';
    return '缺少登录态';
}
function getServiceAdvice(summary) {
    if (summary.sourceKind === 'domain') {
