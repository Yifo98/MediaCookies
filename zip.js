...
export function buildZip(files, modifiedAt = new Date()) {
    const encodedFiles = [];
    const localParts = [];
    let offset = 0;
    files.forEach((file) => {
...
function encodeFile(file, modifiedAt, offset) {
...
function buildLocalHeader(file) {
...
function buildCentralDirectoryHeader(file) {
