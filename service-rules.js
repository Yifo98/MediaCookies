...
const subdomainOrigins = (domains) => domains.flatMap((domain) => [
    `https://${domain}/*`,
    `https://*.${domain}/*`,
]);
...
const SUPPORTED_SITE_BY_KEY = new Map();
const SUPPORTED_DOMAIN_BY_KEY = new Map();
...
const SUPPORTED_DOMAIN_ALIASES = new Map([
    ['b23.tv', 'BiliBili'],
...
export function getServiceRules(slugs, rules = SERVICE_RULES) {
...
export function cookieMatchesRule(cookie, rule) {
    return rule.domains.some((domain) => domainMatches(cookie.domain, domain));
}
function buildYtDlpRule(groupKey) {
...
function buildDomainRule(groupKey) {
    return {
...
export function domainMatches(domain, expected) {
    const normalized = normalizeDomain(domain);
    const target = normalizeDomain(expected);
    return normalized === target || normalized.endsWith(`.${target}`);
}
export function getDomainGroupKey(domain) {
...
function getSupportedSiteForDomain(domain) {
    const groupKey = getDomainGroupKey(domain);
    const alias = SUPPORTED_DOMAIN_ALIASES.get(groupKey);
    if (alias && isSupportedSiteName(alias))
        return alias;
...
function isSupportedSiteName(name) {
    return SUPPORTED_SITE_BY_KEY.has(normalizeSiteKey(name));
}
function getExtractorBaseName(name) {
    return name.split(':')[0].trim();
}
function getRootLabel(groupKey) {
